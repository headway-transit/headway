...
// plus one line of why. Never 777. See the vendorfile twin for the origin
// story (the first live agency install was blocked on exactly this).
...
const wantFix = "sudo chown -R 65532:65532 deploy/compose/tides-drop"
...
func TestUnwritableDropDirPrintsFixForProcessedDir(t *testing.T) {
	...
	s.HostDir = "deploy/compose/tides-drop"
	dropFile(t, dir, "passenger_events_2026-07-30.csv", validCSV)
...
func TestUnreadableDropFilePrintsFix(t *testing.T) {
	...
	s.HostDir = "deploy/compose/tides-drop"
	path := dropFile(t, dir, "passenger_events_x.csv", validCSV)
