...
// dropped (Guardrail 7).
//
// The envelope source is configurable (DR_SOURCE): real dispatch feeds use
// the default "dr"; simulator drops MUST use "dr_simulated" so simulated
// data stays permanently distinguishable in provenance (the handoff-0005
// binding rule, applied to DR by handoff 0013).
//
...
type Scanner struct {
	Dir      string
	Source   string // envelope source; empty means DefaultSource ("dr")
...
// ScanOnce processes every demand_response_trips*.csv currently in Dir, in
...
func (s *Scanner) moveToProcessed(path string) error {
	...
	if err := os.Rename(path, dest); err != nil {
		return fmt.Errorf("dr: move %s to processed: %w", path, err)
	}
	return nil
}
