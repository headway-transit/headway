...
| POST | `/calc/runs` | `data_steward` or above | Dispatch one calculation run (handoff 0026 / migration 0033): the API launches `python -m headway_calc.runner --period-start --period-end` (the CLI's default calc set, same interpreter — the API is a DISPATCHER, no calc logic, no knobs) as a background-thread subprocess and records every status transition in `computed.calc_runs`. Half-open period, validated (start < end, ≤ 1 year). **Single-flight**: one live run at a time, enforced by a partial unique index — a second POST is a 409 naming the live run in plain words (no queue pretensions in v0). Audited `calc_run_requested`. Returns 202; the row, not the response, is the truth. `report_preparer` is included by the recorded decision: computing figures is stewardship, separation-of-duties bites at certification. |
| GET | `/calc/runs` (+ `/{id}`) | any signed-in role | Run history newest first (bounded, `limit` ≤ 200) / one run — the poll target while a run is live. Status vocabulary `queued\|running\|succeeded\|refused\|failed`: a run whose calcs ALL withheld their figures over blocking DQ findings is **`refused` — first-class, the product working** — with the per-calc summary naming persisted figure ids and the exact blocking `dq.issues` ids (the UI links straight to them). Figures in summaries are the runner's own strings, verbatim. **Staleness honesty** (recorded choice): a queued/running row older than 2 h presents `stale: true` + a plain-words note (API likely restarted mid-run — state unknown), stops blocking new runs, and is reconciled to `failed` (audited `calc_run_marked_stale`) on the next POST. |
...
| PUT | `/settings/{key}` | `certifying_official` only | Change one policy setting. Value validated against the setting's `value_type` (decimal parsed via `Decimal` — never float; 422 in plain language); old→new audited; unknown key 404 (settings are seeded, never client-creatable). Brand colors additionally pass the WCAG surface guardrail; `brand_chrome_*` keys (migration 0027) pass the PAIRWISE chrome guardrail against the values that would result. |
| POST | `/sandbox/preview` | any signed-in role | What-if modeling preview (handoff 0017 design point 6): propose values for the seeded policy knobs (four NTD calc knobs and/or the two OTP-window ops knobs) over a period → figure impact vs the CURRENT audited settings, both variants computed over the SAME inputs by the calc library's READ-ONLY preview entry points. **Changes nothing** (no figures, no DQ rows, no commit — pinned by calc test, live-verified by row counts), results are EPHEMERAL and therefore never certifiable; the response banner says so and points at the audited `PUT /settings/{key}` flow for actually applying a knob. Broken settings table → 503 refusal (a baseline can't be guessed). |
...
| GET | `/sampling/plans/{id}/worksheet` | any signed-in role | The ride-check worksheet as CSV/XLSX (`format`): one row per selected unit per draw with its measured state (active measurements only); the plan's requirement, the UNDERSAMPLED/estimate-ready state and the retention note lead. Same export discipline as `/metrics/values/export`. |
...
| GET | `/branding` | **none — public** | `{display_name, primary, accent, has_logo, chrome, chrome_note}` for the app shell — colors already passed the contrast guardrail at write time. `chrome` (branding v2, migration 0027) is `{header_bg, header_fg, accent}` ONLY when the agency set all three `brand_chrome_*` keys (each pair AA-validated pairwise at write time — fg on bg, accent on bg); `null` = neutral Headway chrome. `chrome_note` restates the standing dark-mode limitation. Rate-limited per client IP. |
