...
| DB migrations | **0001–0031 all applied** to the live TimescaleDB (psql-listed 2026-07-12); runner idempotent; immutability proven by attack, incl. safety.events + sampling.* append-only triggers (handoffs 0001/0002/0005/0006/0010/0012) | — |
...
| Installer | Pre-check phase exercised live — correctly diagnosed docker-group and port conflicts (`install/install.log`); **first complete fresh-box guided install 2026-07-22**: a partner agency's ITS manager took a fresh Ubuntu 26.04 Server VM from bare box → running UI with real GTFS + GTFS-Realtime feeds connected (reported by the project lead; two doc/UX gaps found live and fixed same-day: the bare-box prelude in `install/README.md`, exact apt commands in the compose-missing refusal) | The agency run's `install.log` not yet on file (evidence is the project lead's report) — request a copy for the record; `--check-updates`/`--upgrade` from an agency box still unexercised (blocked in part on the ghcr visibility flip) |
